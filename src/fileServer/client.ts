...
function handleServerError(error: any, context: string) {
  if (
    ...
    console.error(
      `❌ Cannot connect to file server. Please make sure the server is running and accessible at ${BASE_URL}`
    );
  } else {
...
async function getFileInfo(filePath: string) {
  throw new Error(
    "Direct file info access is not allowed. Use the server endpoint instead."
  );
}

async function findFiles(searchPath: string, pattern: string) {
  throw new Error(
    "Direct file search is not allowed. Use the server endpoint instead."
  );
}
      ...
      switch (command.toLowerCase()) {
        case "read": {
          if (!filePath) {
            ...
            await logger.log(
              "Error: No file path provided for read command",
              "ERROR"
            );
            break;
          }
          const response = await fetch(
            `${BASE_URL}/read?path=${encodeURIComponent(filePath)}`
          );
          if (!response.ok)
        ...
        case "list": {
          if (!filePath) {
            ...
            await logger.log(
              "Error: No directory path provided for list command",
              "ERROR"
            );
            break;
          }
          const response = await fetch(
            `${BASE_URL}/list?path=${encodeURIComponent(filePath)}`
          );
          if (!response.ok)
            throw new Error(`HTTP error! status: ${response.status}`);
          const result = (await response.json()) as {
          ...
          await logger.log(
            `Successfully listed directory: ${filePath}`,
            "FILE_CLIENT"
          );
          break;
        }
        case "find": {
          if (args.length < 2) {
            ...
            await logger.log(
              "Error: Invalid arguments for find command",
              "ERROR"
            );
            break;
          }
          ...
          const response = await fetch(
            `${BASE_URL}/search?path=${encodeURIComponent(
              searchPath
          ...
          if (result.files.length === 0) {
            ...
            await logger.log(
              `No files found matching pattern "${pattern}" in ${searchPath}`,
              "FILE_CLIENT"
            );
          } else {
            result.files.forEach((file) =>
            ...
            await logger.log(
              `Found ${result.files.length} files matching pattern "${pattern}" in ${searchPath}`,
              "FILE_CLIENT"
            );
          }
          break;
        ...
        case "info": {
          if (!filePath) {
            ...
            await logger.log(
              "Error: No file path provided for info command",
              "ERROR"
            );
            break;
          }
          ...
          if (result.info) {
            console.log(
              `  Type: ${result.info.isDirectory ? "Directory" : "File"}`
            );
