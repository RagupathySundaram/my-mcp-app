...
const API_TIMEOUT_MS = Number(process.env.API_TIMEOUT_MS) || 7000;
...
export async function getLatLon(city: string): Promise<{ lat: number; lon: number } | null> {
	...
	try {
		...
		console.error(`apiServices.getLatLon: fetch failed for ${url}:`, (err as any)?.message || err);
	}
	...
	try {
		...
		if (resp3 && resp3.ok) {
			...
			if (data3.results && data3.results.length > 0) {
				return { lat: data3.results[0].latitude, lon: data3.results[0].longitude };
			}
		}
	} catch (err) {
		console.error(`apiServices.getLatLon: secondary fetch failed for ${city}:`, (err as any)?.message || err);
	}
